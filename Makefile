# Makefile for Go Fiber + GORM Project (lives in ./go/)

APP                := go-fiber-app
MAIN_GO            := main.go
BUILD_DIR          := /tmp
ARTIFACT           := $(APP)-$(shell date -u +%Y%m%d%H%M%S)
...
build: tidy
	@CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o $(BUILD_DIR)/$(ARTIFACT) $(MAIN_GO)
